<template>
  <div ref="chat" class="chat-wrapper">
    <mu-appbar style="width: 100%;" color="primary" class="chat-header">
      ...
      <p>嗨聊({{userInfo.username}})</p>
      <mu-button flat slot="right" @click="logoutOpen = true">注销</mu-button>
    </mu-appbar>
    <mu-dialog title="注销" width="360" :open.sync="logoutOpen">你确定要退出吗？
      ...
      <mu-row>
        <mu-col span="12" sm="12" md="12" lg="12" xl="12" class="chat-body-list">
          ...
          <div class="grid-cell chat-userlist" v-if="listType == 'online'">
            <chat-list
              v-if="onlineUserList.length"
              :user-list="onlineUserList"
              :msg-count="msgCount"
          ...
          <!-- 历史用户列表 -->
          <div
            class="grid-cell chat-userlist"
            v-if="listType == 'history'"
            v-loading="historyUserLoad"
            data-mu-loading-text="skr!疯狂加载中..."
          >
            <chat-list
              v-if="historyUserList.length"
              :user-list="historyUserList"
              :msg-count="msgCount"
              @openDialog="openFullscreenDialog"
            ></chat-list>
          </div>
        ...
        >
          ...
          <mu-appbar
            color="primary"
            :title="'正在和'+talkManInfo.username+'聊天'"
            class="chat-dialog-appbar"
          >
          ...
          >
            ...
            <section class="chat-dialog-msgbox" ref="msgbox">
              <div class="msg-container" v-for="(item,index) in messageList" :key="index">
                <!-- 选项框 -->
                <ul
                  v-show="item.showSelect"
                  class="msg-select"
                  :class="{'msg-select-right':(item.sender == userInfo.username),'msg-select-left':(item.sender != userInfo.username)}"
                >
                  <li @click="quoteMessage(item)">文本引用</li>
                  <li
                    v-if="item.showrecall&&((item.sender != talkManInfo.username))"
                    @click="recallMessage(item,index)"
                  >撤回</li>
                  <li
                    v-if="item.picture == null&&item.showrecall&&((item.sender != talkManInfo.username))"
                    @click="modifyMessage(item,index)"
                  >修改</li>
                </ul>
                <!-- 最新消息提示 -->
                ...
                <!-- 消息框 -->
                <section
                  v-if="item.isrecall== undefined || !item.isrecall"
                  class="msg-msgbox"
                  @click="item.showSelect = false"
                  @dblclick="gtouchstart(item)"
                ...
                >
                  ...
                  <div
                    class="msg-avatar"
                    :class="{'msg-avatar-right':(item.sender==userInfo.username)}"
                  >
                    <img src="@/assets/images/p0.jpg">
                  </div>
                  <div
                    :class="{'msg-text-right':(item.sender==userInfo.username),'msg-text-left':(item.sender != userInfo.username)}"
                  >
            ...
            <div class="chat-dialog-inputbox">
              ...
              <div class="chat-dialog-sendimgbtn">
                发送图片
                <input
                  class="chat-dialog-sendimg"
                  type="file"
                  multiple
                  accept="image/*"
                  @change="uploadImg"
    ...
    <footer class="chat-footer">
      <mu-bottom-nav
        color="#168ad6"
        class="chat-footer-nav"
        :value.sync="listType"
        @change="showUserList(listType)"
      >
        <mu-bottom-nav-item title="在线用户" value="online" icon="group"></mu-bottom-nav-item>
        <mu-bottom-nav-item title="历史列表" value="history" icon="restore"></mu-bottom-nav-item>
      </mu-bottom-nav>
    </footer>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    closeFullscreenDialog() {
      ...
      this.msgCount.push('flag');
      this.msgCount.pop('flag');
      // 清除聊天人信息
      this.talkManInfo = {};
      // 清除聊天信息
      this.messageList.length = 0;
